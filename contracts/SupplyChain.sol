...
contract SupplyChain {
  ...
  */
  struct Item{
    ...
    uint price;
    uint state;
    address payable seller; //string seller if names are printed
    address payable buyer;//string buyer if names are printed
  }
...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier verifyCaller (address _address) {
    require(msg.sender == _address,"verified address");
     _;
  }
  ...
  modifier forSale(uint _skuCount){
    require(items[_skuCount].state == uint(State.ForSale), "item is available");
    _;
  }
  modifier sold (uint _skuCount){
    require(items[_skuCount].state == uint(State.Sold),"item is sold");
    _;
  }
  modifier shipped(uint _skuCount){
    require(items[_skuCount].state == uint(State.Shipped), "Item is Shipped");
    _;
  }
  modifier received(uint _skuCount){
    require(items[_skuCount].state == uint(State.Received),"Item received");
    _;
  }
